...
const getUserOrder = asyncHandler(async (req, res, next) => {
    ...
    const filteredOrders = orders.filter(
        (order) => order.store?.status === "APPROVED"
    );
    ...
    const shipMap = Object.fromEntries(
        shipInfos.map((info) => [info.orderId.toString(), info])
    );
...
const updateOrderStatus = asyncHandler(async (req, res, next) => {
    ...
    if (status === currentStatus) {
        return next(
            createError(400, `Order is already in '${status}' status.`)
        );
    }
...
const reOrder = async (req, res) => {
    try {
        ...
        const hasOutOfStock = order.items.some(
            (item) => item.dish?.stockStatus === "OUT_OF_STOCK"
        );
